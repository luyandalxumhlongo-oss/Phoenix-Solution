...
def render() :
    ...
    if lang == "1":
        ...
        while(True):
         ...
         print("Sawubona mina ngingu Thuma Mina i'chatbot' ongakhuluma nayo"
              "\n Description"
              )
         ...
         while(True):
            iD_no = input("\n Bhala inombolo yakho yeID (umazisi) : ").strip()

            if database.users:
                person = database.get_user(iD_no)

            if not database.get_user(iD_no): #ID_no not in Database
                res2 = input("\n Uyafisa ukungenela kulelnkonzo lezimali?\n 1.Yebbo \n 2.Cha ").strip()

                if (res2 == "2") :
                    print("Siyabonga ngepundulo.")
                    continue

                else:
                    name = input("Bhala igama lakho: ").strip()
                    surname = input("Bhala isibongo sakho: ").strip()
                    cell_num = input("Bhala inombolo yakho yecingo: ").strip()
                    user_name = name + iD_no[6:]
                    person = user(user_name , name, surname, iD_no, cell_num )
                    database.add_user(person)
                    print("Halala usukwazile ukungenela lenkonzo yemali, usuyilunga lethu manje.")
                    continue

            while(True):
                choice = input(
                    "\n1.Bheka ibhalansi  2.Faka amakhredithi  3.Thumela amakhredithi "
                    "4.Buka iphrofayili yakho   5.Ofuna usizo   6.Phuma ohlelweni\nCindezela inombolo: "
                ).strip()
                ...
                if choice == "1":
                    ...
                    vou = input("Faka inombolo yevawusha yakho: ").strip()
                    if vou in person.vouchers:
                        person.add_credits(person.vouchers[vou])
                        person.vouchers.pop(vou)
                        print("ivawusha isingenile.")
                    else:
                        print("Ivawusha engavumelekile.")

                elif choice == "3":
                    u_name = input("Faka igama lomsebenzisi womamukeli ").strip()
                    amt = int(input("Faka inani: "))
                    person.transfer(u_name, amt)

                elif choice == "4":
                    print(f"Igama lomsebenzisi : {person.user_name}"
                          f"\nIgama lakho      : {person.name}"
                          f"\nInombolo yomazisi : {person.iD_number}")

                elif choice == "5":
                    print("Ukuze uthole usizo Cindezela u: Request agent")

                elif choice == "6":
                    print("Usuphumile ohlelweni...")
                    break

                else:
                    print("Ucindezela inombolo engavumelekile.")
